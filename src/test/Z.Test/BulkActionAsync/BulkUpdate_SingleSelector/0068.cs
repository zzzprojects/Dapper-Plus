    ...
    public partial class BulkActionAsync_BulkUpdate_SingleSelector
    {
		...
		public async Task Z_Test_0068()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate("key", single1, x => x.Many1));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + many1.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
    ...
    public partial class BulkActionAsync_BulkDelete_ManySelector
    {
		...
		public async Task Z_Test_0128()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, x => x.Many2, x => x.Many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) - many1.Sum(x => x.Many3.Sum(y => y.ColumnInt)), columnInt_Key);

    ...
    public partial class BulkActionAsync_BulkMerge_SingleMixedSelector
    {
		...
		public async Task Z_Test_0178()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge(single1, x => x.Single2, x => x.Many2, x => x.Many3));
            }
			...
			Assert.AreEqual(columnInt_before + single1.ColumnInt, columnInt);
					Assert.AreEqual(single2.ColumnInt + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnUpdateInt);

    ...
    public partial class BulkActionAsync_BulkDelete_ManyMixedSelector
    {
		...
		public async Task Z_Test_0144()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, x => x.Many2, x => x.Single2, x=> x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) - many1.Sum(x => x.Single2.ColumnInt) - many1.Sum(x => x.Single3.ColumnInt), columnInt_Key);

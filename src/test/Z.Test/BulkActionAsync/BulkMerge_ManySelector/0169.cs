    ...
    public partial class BulkActionAsync_BulkMerge_ManySelector
    {
		...
		public async Task Z_Test_0169()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge(many1, x => x.Single1, x => x.Single2, x => x.Single3));
            }
            ...
            // Test
			Assert.AreEqual(columnInt_before + many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Single1.ColumnInt), columnInt);
					Assert.AreEqual(many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Single3.ColumnInt), columnUpdateInt);

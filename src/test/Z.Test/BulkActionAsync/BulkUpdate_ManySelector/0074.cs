    ...
    public partial class BulkActionAsync_BulkUpdate_ManySelector
    {
		...
		public async Task Z_Test_0074()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(many1, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnUpdateInt);

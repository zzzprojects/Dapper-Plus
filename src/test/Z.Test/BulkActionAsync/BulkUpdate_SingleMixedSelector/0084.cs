    ...
    public partial class BulkActionAsync_BulkUpdate_SingleMixedSelector
    {
		...
		public async Task Z_Test_0084()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(single1, x => x.Many2, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(single1.ColumnInt + many2.Sum(x => x.ColumnInt) + single2.ColumnInt + single3.ColumnInt, columnUpdateInt);

    ...
    public partial class BulkActionAsync_BulkUpdate_ManyMixedSelector
    {
		...
		public async Task Z_Test_0091()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(many1, x => x.Many2, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) + many1.Sum(x => x.Single2.ColumnInt), columnUpdateInt);

    ...
    public partial class BulkActionAsync_BulkDelete_SingleMixedSelector
    {
		...
		public async Task Z_Test_0136()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", single1, x => x.Many2, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - single1.ColumnInt - many2.Sum(x => x.ColumnInt) - single2.ColumnInt - single3.ColumnInt, columnInt_Key);

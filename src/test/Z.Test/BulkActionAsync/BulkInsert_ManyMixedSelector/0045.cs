    ...
    public partial class BulkActionAsync_BulkInsert_ManyMixedSelector
    {
		...
		public async Task Z_Test_0045()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert("key", many1, x => x.Single2, x => x.Many2));
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnInt_Key);

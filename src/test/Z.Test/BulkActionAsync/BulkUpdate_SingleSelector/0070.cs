...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkUpdate_SingleSelector
    {
		[TestMethod]
		public async Task Z_Test_0070()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate("key", single1, x => x.Many1, x => x.Many2, x => x.Many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
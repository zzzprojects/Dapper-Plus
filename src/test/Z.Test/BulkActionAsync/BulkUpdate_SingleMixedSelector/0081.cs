    ...
    public partial class BulkActionAsync_BulkUpdate_SingleMixedSelector
    {
		...
		public async Task Z_Test_0081()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(single1, x => x.Single2, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(single1.ColumnInt + single2.ColumnInt + many2.Sum(x => x.ColumnInt), columnUpdateInt);

    ...
    public partial class BulkActionAsync_BulkMerge_ManyMixedSelector
    {
		...
		public async Task Z_Test_0191()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", many1, x => x.Many2, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) + many1.Sum(x => x.Single2.ColumnInt), columnUpdateInt_Key);
		}
    }
}
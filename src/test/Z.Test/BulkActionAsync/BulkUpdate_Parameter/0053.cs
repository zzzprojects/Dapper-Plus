    ...
    public partial class BulkActionAsync_BulkUpdate_Parameter
    {
		...
		public async Task Z_Test_0053()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(many1, many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt), columnUpdateInt);

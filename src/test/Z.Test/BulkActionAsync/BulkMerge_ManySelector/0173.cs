    ...
    public partial class BulkActionAsync_BulkMerge_ManySelector
    {
		...
		public async Task Z_Test_0173()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", many1, x => x.Single1, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.Single2.ColumnInt), columnUpdateInt_Key);
		}
    }
}
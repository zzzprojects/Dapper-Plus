    ...
    public partial class BulkActionAsync_BulkInsert_ManyMixedSelector
    {
		...
		public async Task Z_Test_0044()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(many1, x => x.Many2, x => x.Single2, x=> x.Single3));
            }
            ...
            // Test
			Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) + many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Single3.ColumnInt), columnInt);

    ...
    public partial class BulkActionAsync_BulkMerge_ManyMixedSelector
    {
		...
		public async Task Z_Test_0185()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge(many1, x => x.Single2, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before + many1.Sum(x => x.ColumnInt), columnInt);
					Assert.AreEqual(many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnUpdateInt);

    ...
    public partial class BulkActionAsync_BulkInsert_ManySelector
    {
		...
		public async Task Z_Test_0026()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(many1, x => x.Many2));
            }
            ...
            // Test
			Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnInt);

    ...
    public partial class BulkActionAsync_BulkDelete_ManySelector
    {
		...
		public async Task Z_Test_0125()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, x => x.Single1, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many1.Sum(x => x.Single1.ColumnInt) - many1.Sum(x => x.Single2.ColumnInt), columnInt_Key);

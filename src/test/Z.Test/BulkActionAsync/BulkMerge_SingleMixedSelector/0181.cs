    ...
    public partial class BulkActionAsync_BulkMerge_SingleMixedSelector
    {
		...
		public async Task Z_Test_0181()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", single1, x => x.Single2, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single2.ColumnInt + many2.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
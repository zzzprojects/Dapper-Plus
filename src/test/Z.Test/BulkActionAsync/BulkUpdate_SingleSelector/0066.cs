    ...
    public partial class BulkActionAsync_BulkUpdate_SingleSelector
    {
		...
		public async Task Z_Test_0066()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate("key", single1, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + single2.ColumnInt, columnUpdateInt_Key);
		}
    }
}
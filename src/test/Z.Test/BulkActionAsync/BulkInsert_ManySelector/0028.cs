    ...
    public partial class BulkActionAsync_BulkInsert_ManySelector
    {
		...
		public async Task Z_Test_0028()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert("key", many1, x => x.Single1));
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.ColumnInt)+many1.Sum(x => x.Single1.ColumnInt), columnInt_Key);

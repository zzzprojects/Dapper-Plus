    ...
    public partial class BulkActionAsync_BulkMerge_SingleSelector
    {
		...
		public async Task Z_Test_0160()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge(single1, x => x.Many1, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before + single1.ColumnInt + many1.Sum(x => x.ColumnInt), columnInt);
					Assert.AreEqual(many2.Sum(x => x.ColumnInt), columnUpdateInt);

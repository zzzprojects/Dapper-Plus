    ...
    public partial class BulkActionAsync_BulkDelete_SingleMixedSelector
    {
		...
		public async Task Z_Test_0129()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete(single1, x => x.Single2, x => x.Many2));
            }
            ...
            // Test
			Assert.AreEqual(columnInt_before - single1.ColumnInt - single2.ColumnInt - many2.Sum(x => x.ColumnInt), columnInt);

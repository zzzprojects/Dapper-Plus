    ...
    public partial class BulkActionAsync_BulkInsert_ManyMixedSelector
    {
		...
		public async Task Z_Test_0041()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(many1, x => x.Single2, x => x.Many2));
            }
            ...
            // Test
			Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnInt);

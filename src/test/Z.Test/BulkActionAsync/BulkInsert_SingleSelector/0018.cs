    ...
    public partial class BulkActionAsync_BulkInsert_SingleSelector
    {
		...
		public async Task Z_Test_0018()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert("key", single1, x => x.Single2));
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + single2.ColumnInt, columnInt_Key);

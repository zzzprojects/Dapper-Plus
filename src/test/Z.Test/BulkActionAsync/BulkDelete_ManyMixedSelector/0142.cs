...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkDelete_ManyMixedSelector
    {
		[TestMethod]
		public async Task Z_Test_0142()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, x => x.Single2, x => x.Many2, x=> x.Many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many1.Sum(x => x.Single2.ColumnInt) - many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) - many1.Sum(x => x.Many3.Sum(y => y.ColumnInt)), columnInt_Key);

...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkInsert_SingleSelector
    {
		[TestMethod]
		public async Task Z_Test_0017()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(single1, x => x.Many1, x => x.Many2, x => x.Many3));
            }
            ...
            // Test
			Assert.AreEqual(single1.ColumnInt + many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnInt);

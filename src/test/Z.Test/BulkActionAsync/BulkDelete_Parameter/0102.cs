    ...
    public partial class BulkActionAsync_BulkDelete_Parameter
    {
		...
		public async Task Z_Test_0102()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete(many1, many2, many3));
            }
            ...
            // Test
			Assert.AreEqual(columnInt_before - many1.Sum(x => x.ColumnInt) - many2.Sum(x => x.ColumnInt) - many3.Sum(x => x.ColumnInt), columnInt);

    ...
    public partial class Bulk_BulkAction_BulkInsert_SingleMixedSelector
    {
		...
		public void Z_Test_0040()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                cn.BulkInsert((SingleMany)null).BulkInsert("key", single1, x => x.Many2, x => x.Single2, x => x.Single3);
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + many2.Sum(x => x.ColumnInt) + single2.ColumnInt + single3.ColumnInt, columnInt_Key);

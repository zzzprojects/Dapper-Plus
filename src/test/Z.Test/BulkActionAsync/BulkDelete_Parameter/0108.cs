...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkDelete_Parameter
    {
		[TestMethod]
		public async Task Z_Test_0108()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, many2, many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many2.Sum(x => x.ColumnInt) - many3.Sum(x => x.ColumnInt), columnInt_Key);

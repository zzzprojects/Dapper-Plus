    ...
    public partial class BulkActionAsync_BulkMerge_ManyMixedSelector
    {
		...
		public async Task Z_Test_0189()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", many1, x => x.Single2, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnUpdateInt_Key);
		}
    }
}
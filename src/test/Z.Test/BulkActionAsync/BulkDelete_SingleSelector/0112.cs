    ...
    public partial class BulkActionAsync_BulkDelete_SingleSelector
    {
		...
		public async Task Z_Test_0112()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete(single1, x => x.Many1, x => x.Many2));
            }
            ...
            // Test
			Assert.AreEqual(columnInt_before - single1.ColumnInt - many1.Sum(x => x.ColumnInt) - many2.Sum(x => x.ColumnInt), columnInt);

    ...
    public partial class BulkActionAsync_BulkDelete_ManyMixedSelector
    {
		...
		public async Task Z_Test_0137()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete(many1, x => x.Single2, x => x.Many2));
            }
            ...
            // Test
			Assert.AreEqual(columnInt_before - many1.Sum(x => x.ColumnInt) - many1.Sum(x => x.Single2.ColumnInt) - many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)), columnInt);

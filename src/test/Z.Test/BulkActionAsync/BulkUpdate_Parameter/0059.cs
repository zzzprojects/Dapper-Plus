    ...
    public partial class BulkActionAsync_BulkUpdate_Parameter
    {
		...
		public async Task Z_Test_0059()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate("key", many1, many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
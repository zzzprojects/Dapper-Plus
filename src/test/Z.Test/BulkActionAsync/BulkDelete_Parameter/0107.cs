    ...
    public partial class BulkActionAsync_BulkDelete_Parameter
    {
		...
		public async Task Z_Test_0107()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", many1, many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - many1.Sum(x => x.ColumnInt) - many2.Sum(x => x.ColumnInt), columnInt_Key);

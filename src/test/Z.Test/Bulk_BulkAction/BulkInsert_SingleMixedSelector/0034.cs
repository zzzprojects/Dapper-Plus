...
namespace Z.Test
{
    public partial class Bulk_BulkAction_BulkInsert_SingleMixedSelector
    {
		[TestMethod]
		public void Z_Test_0034()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                cn.BulkInsert((SingleMany)null).BulkInsert(single1, x => x.Single2, x => x.Many2, x => x.Many3);
            }
            ...
            // Test
			Assert.AreEqual(single1.ColumnInt + single2.ColumnInt + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnInt);

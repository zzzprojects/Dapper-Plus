    ...
    public partial class BulkActionAsync_BulkDelete_SingleSelector
    {
		...
		public async Task Z_Test_0117()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkDelete("key", single1, x => x.Many1, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before - single1.ColumnInt - many1.Sum(x => x.ColumnInt) - many2.Sum(x => x.ColumnInt), columnInt_Key);

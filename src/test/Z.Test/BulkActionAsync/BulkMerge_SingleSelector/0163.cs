    ...
    public partial class BulkActionAsync_BulkMerge_SingleSelector
    {
		...
		public async Task Z_Test_0163()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", single1, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single2.ColumnInt + single3.ColumnInt, columnUpdateInt_Key);
		}
    }
}
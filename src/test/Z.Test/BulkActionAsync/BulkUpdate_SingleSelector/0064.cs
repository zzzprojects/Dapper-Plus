    ...
    public partial class BulkActionAsync_BulkUpdate_SingleSelector
    {
		...
		public async Task Z_Test_0064()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(single1, x => x.Many1, x => x.Many2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(single1.ColumnInt + many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt), columnUpdateInt);

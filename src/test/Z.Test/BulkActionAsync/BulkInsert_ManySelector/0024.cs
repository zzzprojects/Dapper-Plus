    ...
    public partial class BulkActionAsync_BulkInsert_ManySelector
    {
		...
		public async Task Z_Test_0024()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(many1, x => x.Single1, x => x.Single2));
            }
            ...
            // Test
			Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Single1.ColumnInt) + many1.Sum(x => x.Single2.ColumnInt), columnInt);

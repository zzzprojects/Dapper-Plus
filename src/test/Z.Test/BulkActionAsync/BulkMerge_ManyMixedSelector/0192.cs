...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkMerge_ManyMixedSelector
    {
		[TestMethod]
		public async Task Z_Test_0192()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", many1, x => x.Many2, x => x.Single2, x=> x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.Many2.Sum(y => y.ColumnInt)) + many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Single3.ColumnInt), columnUpdateInt_Key);
		}
    }
}
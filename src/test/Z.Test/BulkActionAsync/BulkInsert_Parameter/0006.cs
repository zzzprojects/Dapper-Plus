...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkInsert_Parameter
    {
		[TestMethod]
		public async Task Z_Test_0006()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert(many1, many2, many3));
            }
            ...
            // Test
			Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnInt);

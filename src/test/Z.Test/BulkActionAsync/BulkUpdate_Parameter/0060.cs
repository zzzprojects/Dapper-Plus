...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkUpdate_Parameter
    {
		[TestMethod]
		public async Task Z_Test_0060()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate("key", many1, many2, many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
    ...
    public partial class BulkActionAsync_BulkInsert_Parameter
    {
		...
		public async Task Z_Test_0012()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert("key", many1, many2, many3));
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnInt_Key);

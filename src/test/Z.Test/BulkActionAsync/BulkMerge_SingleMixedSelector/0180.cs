    ...
    public partial class BulkActionAsync_BulkMerge_SingleMixedSelector
    {
		...
		public async Task Z_Test_0180()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge(single1, x => x.Many2, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before + single1.ColumnInt, columnInt);
					Assert.AreEqual(many2.Sum(x => x.ColumnInt) + single2.ColumnInt + single3.ColumnInt, columnUpdateInt);

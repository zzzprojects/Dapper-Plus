...
namespace Z.Test
{
    public partial class BulkActionAsync_BulkMerge_SingleMixedSelector
    {
		[TestMethod]
		public async Task Z_Test_0182()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", single1, x => x.Single2, x => x.Many2, x => x.Many3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single2.ColumnInt + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
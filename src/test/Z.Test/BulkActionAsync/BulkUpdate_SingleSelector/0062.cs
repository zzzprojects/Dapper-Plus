    ...
    public partial class BulkActionAsync_BulkUpdate_SingleSelector
    {
		...
		public async Task Z_Test_0062()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(single1, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(single1.ColumnInt + single2.ColumnInt + single3.ColumnInt, columnUpdateInt);

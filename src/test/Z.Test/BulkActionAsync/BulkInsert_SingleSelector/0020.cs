    ...
    public partial class BulkActionAsync_BulkInsert_SingleSelector
    {
		...
		public async Task Z_Test_0020()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkInsert("key", single1, x => x.Many1));
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + many1.Sum(x => x.ColumnInt), columnInt_Key);

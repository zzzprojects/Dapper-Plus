    ...
    public partial class Bulk_BulkAction_BulkInsert_SingleMixedSelector
    {
		...
		public void Z_Test_0038()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                cn.BulkInsert((SingleMany)null).BulkInsert("key", single1, x => x.Single2, x => x.Many2, x => x.Many3);
            }
			...
			Assert.AreEqual(0, columnInt);
					...
					Assert.AreEqual(single1.ColumnInt + single2.ColumnInt + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnInt_Key);

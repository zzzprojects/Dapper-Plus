    ...
    public partial class BulkActionAsync_BulkMerge_SingleSelector
    {
		...
		public async Task Z_Test_0162()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkMerge("key", single1, x => x.Single2));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(single2.ColumnInt, columnUpdateInt_Key);
		}
    }
}
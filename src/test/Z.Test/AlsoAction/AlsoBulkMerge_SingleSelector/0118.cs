...
namespace Z.Test
{
    public partial class AlsoAction_AlsoBulkMerge_SingleSelector
    {
		[TestMethod]
		public void Z_Test_0118()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                cn.BulkMerge("key", single1).AlsoBulkMerge("key", x => x.Many1, x => x.Many2, x => x.Many3);
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					...
					Assert.AreEqual(columnInt_Key_before + single1.ColumnInt + many1.Sum(x => x.ColumnInt), columnInt_Key);
					Assert.AreEqual(columnUpdateInt_Key_before + many2.Sum(x => x.ColumnInt) + many3.Sum(x => x.ColumnInt), columnUpdateInt_Key);
		}
    }
}
    ...
    public partial class BulkActionAsync_BulkUpdate_ManySelector
    {
		...
		public async Task Z_Test_0073()
		{
            ...
            using (var cn = Helper.GetConnection())
            {
                ...
                // Action
                await cn.BulkActionAsync(action => action.BulkUpdate(many1, x => x.Single1, x => x.Single2, x => x.Single3));
            }
			...
			Assert.AreEqual(columnInt_before, columnInt);
					Assert.AreEqual(many1.Sum(x => x.ColumnInt) + many1.Sum(x => x.Single1.ColumnInt) + many1.Sum(x => x.Single2.ColumnInt) + many1.Sum(x => x.Single3.ColumnInt), columnUpdateInt);
